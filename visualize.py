...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--predictions', '-p',
        type=str,
        # default='poss_predictions',
        default='valid_predictions_cutmix',
        required=False,
    ...
    if not FLAGS.ignore_semantics:
        if FLAGS.predictions is not None:
            ...
            label2_paths = os.path.join(FLAGS.predictions, "sequences",
                                       FLAGS.sequence, "predictions")

            label1_paths = 'diff/predictions'
        ...
        label2_names = [os.path.join(dp, f) for dp, dn, fn in os.walk(
            os.path.expanduser(label2_paths)) for f in fn]
        label2_names.sort()


        label1_names = [os.path.join(dp, f) for dp, dn, fn in os.walk(
            os.path.expanduser(label1_paths)) for f in fn]
        label1_names.sort()
    ...
    if FLAGS.ignore_semantics:
        ...
        scan1 = SemLaserScan(color_dict, project=True)

        scan2 = SemLaserScan(color_dict, project=True)
    ...
    if FLAGS.what == "poss":
        ...
        vis = LaserScanVis(scan=scan,
                       ...
                       scan_names=scan_names,
                       label_names=label_names,
                       label1_names=label2_names,
                       label2_names=label1_names,
                       offset=FLAGS.offset,
                       semantics=semantics,
                       instances=False)
    # print instructions
