...
class LaserScanVis:
  ...
  def __init__(self, scan, scan1, scan2,scan_names, label_names, label1_names, label2_names, offset=0,
    ...
    self.label1_names = label1_names
    self.label2_names = label2_names
    self.offset = offset
  ...
  def reset(self):
    ...
    if self.semantics:
      ...
      self.sem_view = vispy.scene.widgets.ViewBox(
          border_color='black', parent=self.canvas.scene)
      self.grid.add_widget(self.sem_view, 0, 2)
      self.sem_vis1 = visuals.Markers()
      self.sem_view.camera = 'turntable'
      self.sem_view.add(self.sem_vis1)
      visuals.XYZAxis(parent=self.sem_view.scene)
      # self.sem_view.camera.link(self.scan_view.camera)
    ...
    if self.semantics:
      ...
      self.sem_view = vispy.scene.widgets.ViewBox(
          border_color='black', parent=self.canvas.scene)
      self.grid.add_widget(self.sem_view, 0, 3)
      self.sem_vis2 = visuals.Markers()
      self.sem_view.camera = 'turntable'
      self.sem_view.add(self.sem_vis2)
      visuals.XYZAxis(parent=self.sem_view.scene)
      # self.sem_view.camera.link(self.scan_view.camera)
  ...
  def update_scan(self):
    ...
    self.scan1.open_scan(self.scan_names[self.offset])

    self.scan2.open_scan(self.scan_names[self.offset])
    ...
    if self.semantics:
      ...
      self.scan1.open_label(self.label1_names[self.offset])
      self.scan1.colorize()

      self.scan2.open_label(self.label2_names[self.offset])
      self.scan2.colorize()
    ...
    if self.semantics:
      ...
      self.sem_vis1.set_data(self.scan1.points,
                            face_color=self.scan1.sem_label_color[..., ::-1],
                            edge_color=self.scan1.sem_label_color[..., ::-1],
                            size=1)

      self.sem_vis2.set_data(self.scan2.points,
                             face_color=self.scan2.sem_label_color[..., ::-1],
                             edge_color=self.scan2.sem_label_color[..., ::-1],
                             size=1)
