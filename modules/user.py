...
class User():
  ...
  def infer(self):
    ...
    total_time = np.sum(cnn)
    total_time_knn = np.sum(cnn) + np.sum(knn)
    total_frames = len(cnn)
    fps = total_frames / total_time if total_time != 0 else 0
    fps_knn = total_frames / total_time_knn if total_time_knn != 0 else 0
    print("Frames Per Second (FPS):{}".format(fps))
    print("Frames Per Second (FPS_knn):{}".format(fps_knn))
