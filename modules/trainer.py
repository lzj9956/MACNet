...
#         self.loss_w = np.power(self.loss_w, power_value) * np.power(10, 1 - power_value)
        ...
        with torch.no_grad():
            ...
            if self.ARCH["train"]["pipeline"] == "scnn":
                from modules.network.fast_scnn import FastSCNN
                self.model = FastSCNN(self.parser.get_n_classes(), aux=True)
...
#             self.info = w_dict['info']
    ...
    def make_log_img(depth, mask, pred, gt, color_fn):
        ...
        # make range image (normalized to 0,1 for saving)归一化
        #print(depth.shape)

        depth = (cv2.normalize(depth, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F) * 255.0).astype(np.uint8)

        out_img = cv2.applyColorMap(depth, Trainer.get_mpl_colormap('viridis')) * mask[..., None]
        ...
        # 拼接mask
        # mask_color = color_fn(mask)
        # out_img = np.concatenate([out_img, gt_color, mask_color], axis=0)
    ...
    def train(self):
        #python几大数据类型:元祖，列表，字典
        #创建一个忽略类别的列表
        self.ignore_class = []
        #如果点云所属类别数的比例小于1e-10，则说明该类别点云数较少，即忽略掉
        for i, w in enumerate(self.loss_w):
            if w < 1e-10:
                self.ignore_class.append(i)
                print("Ignoring class ", i, " in IoU evaluation")
        #传进去三个参数，分别为点云类别数，GPU, 忽略的具体类别
        ...
        # train for n epochs   这是训练100个epoch
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in tqdm(enumerate(train_loader), total=len(train_loader)):
            ...
            with torch.cuda.amp.autocast():
                ...
                if self.ARCH["train"]["aux_loss"]:
                    ...
                    proj_labels = torch.unsqueeze(proj_labels, dim=1)
                    # output = torch.max(output, 1)
                    # output, _ = output.max(dim=1, keepdim=True)
                    output = torch.argmax(output, dim=1, keepdim=True)
                    loss = mseloss(output, proj_labels)
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in tqdm(enumerate(val_loader), total=len(val_loader)):
                ...
                #多个监督函数共同组成损失函数，在train过程还有一个边界损失函数
                loss = wce + jacc
